...
function handleBackgroundDimChange(event) {
    ...
    localStorage.setItem(BG_DIM_KEY, String(value));
...
function handleBackgroundBlurChange(event) {
    const value = Number(event.target.value);
    localStorage.setItem(BG_BLUR_KEY, String(value));
...
function clearBackgroundImage() {
    if (!localStorage.getItem(BG_IMAGE_KEY)) {
        showToast('No background image set', 'info');
        return;
    }
